<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit: {{ $post->title }}</h2>
    </x-slot>
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" placeholder="Leave empty to auto-generate"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="author_name" id="author_name" value="{{ old('author_name', $post->author_name) }}" maxlength="100"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                            <div>
                                ...
                                <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $post->published_at?->format('Y-m-d\TH:i')) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <label class="flex items-center gap-1.5 mt-1 text-sm">
                                    <input type="checkbox" name="clear_published_at" value="1">
                                    <span class="text-gray-500">Clear date</span>
                                </label>
                        ...
                        <div>
                            <x-image-upload name="featured_image" label="Featured Image"
                                preview="{{ $post->getFirstMedia('featured')?->getUrl('thumb') }}" />

                            @if ($post->getFirstMedia('featured'))
                                <label class="flex items-center gap-1.5 text-sm mt-2">
                        ...
                        <div>
                            ...
                            <textarea name="excerpt" id="excerpt" rows="2" maxlength="500"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('excerpt', $post->excerpt) }}</textarea>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $post->meta_title) }}" maxlength="160" placeholder="Defaults to post title"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" id="meta_description" rows="2" maxlength="320"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('meta_description', $post->meta_description) }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $post->meta_keywords) }}" maxlength="500"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="schema_type" id="schema_type"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="BlogPosting" @selected(old('schema_type', $post->schema_type) === 'BlogPosting')>BlogPosting</option>
                                    <option value="Article" @selected(old('schema_type', $post->schema_type) === 'Article')>Article</option>
                                    <option value="TechArticle" @selected(old('schema_type', $post->schema_type) === 'TechArticle')>TechArticle</option>
                                    <option value="NewsArticle" @selected(old('schema_type', $post->schema_type) === 'NewsArticle')>NewsArticle</option>
                                </select>
                            </div>
                            <div>
                                <label for="sitemap_priority" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sitemap Priority</label>
                                <input type="number" name="sitemap_priority" id="sitemap_priority" value="{{ old('sitemap_priority', $post->sitemap_priority) }}" min="0" max="1" step="0.1"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div>
                                <label for="sitemap_changefreq" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Sitemap Change Frequency</label>
                                <select name="sitemap_changefreq" id="sitemap_changefreq"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="monthly" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'monthly')>Monthly</option>
                                    <option value="always" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'always')>Always</option>
                                    <option value="hourly" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'hourly')>Hourly</option>
                                    <option value="daily" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'daily')>Daily</option>
                                    <option value="weekly" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'weekly')>Weekly</option>
                                    <option value="yearly" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'yearly')>Yearly</option>
                                    <option value="never" @selected(old('sitemap_changefreq', $post->sitemap_changefreq) === 'never')>Never</option>
                                </select>
                            </div>
                            <div class="flex flex-wrap items-end gap-6 pb-1">
                                ...
                                <label class="flex items-center gap-2">
                                    <input type="checkbox" name="include_in_sitemap" value="1" @checked(old('include_in_sitemap', $post->include_in_sitemap))
                                        class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">In Sitemap</span>
                                </label>
                                <label class="flex items-center gap-2">
                                    <input type="checkbox" name="include_in_feed" value="1" @checked(old('include_in_feed', $post->include_in_feed))
                                        class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 shadow-sm focus:ring-indigo-500">
